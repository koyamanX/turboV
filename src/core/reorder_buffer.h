...
struct reorder_buffer_t {
	...
	Value1[32];
	Jump1;
#define SIZEOF_REORDER_BUFFER_T 208
};
declare reorder_buffer {
	...
	func_in push(valid0, pc0, inst0, dest0, jump0, valid1, pc1, inst1, dest1, jump1): tag;
	func_in flush();
	func_out full();
	...
	func_out commit(commit_Dest0, commit_Value0, commit_Jump0, commit_PC0, commit_Dest1, commit_Value1, commit_Jump1, commit_PC1);
